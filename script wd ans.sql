...
show tables;
select first_name,last_name,gender from patients where gender='M'; #1

select * from patients where patient_id in(1,5,8); #2

select first_name from patients where first_name like 'T%'; #3

select first_name,last_name from patients where weight between 100 and 120; #4

select concat(first_name," ",last_name) as full_name from patients; #5

select count(birth_date) from patients where birth_date like '2010%'; #6

select first_name,last_name,province_name from patients join province_names on 
patients.province_id = province_names.province_id; #7


select count(patient_id) as Total_Admissions from admissions; #8

select count(patient_id) as Total_admissions from admissions where patient_id = 1; #9

select first_name,last_name,birth_date from patients where height>160 and weight>70; #10

select distinct(birth_date) from patients order by birth_date asc; #11

select first_name from patients group by(first_name) having count(first_name)=1; #12

select patient_id,first_name from patients where first_name like 't____%t'; #13

select first_name from patients order by length(first_name); #14

select sum(gender='M') as Male, sum(gender='F') as Female from patients; #15

select city,count(*) as cc from patients group by city order by cc desc,city asc; #16

select first_name,last_name,birth_date from patients where year (birth_date) between 1970 and 1979 
order by birth_date; #17

select (max(weight)-min(weight)) as wdata from patients where province_id = 'TX' ; #18

select first_name,last_name,'patient' as role from patients union all 
select first_name,last_name,'Doctor' as role from doctors;    #19

select patient_id,first_name,last_name from patients where patient_id not in (select patient_id from admissions); #20
